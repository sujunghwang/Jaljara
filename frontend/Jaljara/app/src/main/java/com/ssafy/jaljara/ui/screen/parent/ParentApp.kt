...
fun ParentNavigationBar(
    items : List<ParentScreen> = listOf(ParentScreen.Start,ParentScreen.Fuck,ParentScreen.Hate),
...
fun ParentApp(
    ...
    val currentScreen = ParentScreen.valueOf(
        backStackEntry?.destination?.route ?: ParentScreen.Start.name
    )
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(route = ParentScreen.Fuck.name) {
                SetTimeScreen()
            }
            composable(route = ParentScreen.Hate.name) {
                SleepCalenderScreen()
            }
        }
    }
